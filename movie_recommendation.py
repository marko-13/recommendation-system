...
def score_based_recommender(movies_ranked):
    ...
    plt.savefig('log/best_movies_by_score.png')
    print(movies_ranked['original_title'].head(10))
...
def description_based_recommender(movies_clean):
    ...
    def give_rec(title, sig=sig):
        ...
        # print("\n\n\nMOVIES CLEAN:")
        plot_movies = movies_clean.iloc[movie_indices]

        plt.figure(figsize=(16, 6))

        ax = sns.barplot(x=plot_movies['weighted_average'].head(10), y=plot_movies['original_title'].head(10), data=plot_movies,
                         palette='deep')

        plt.xlim(2.75, 9.35)
        plt.title('Movies by description', weight='bold')
        plt.xlabel('Weighted Average Score', weight='bold')
        plt.ylabel('Movie Title', weight='bold')

        plt.savefig('log/best_movies_by_description.png')

        print(movies_clean['original_title'].iloc[movie_indices].head(10))
