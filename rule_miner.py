...
def run_fp_growth(df, first_form_ind, target_form_ind):
    ...
    _save_rules(res_df, label_col_name, 'all')
    ...
    _save_rules(res_df, label_col_name, 'relevant')
...
def extract_relevant_itemsets(rules_df, form_name):
    ...
    for index, row in rules_df.iterrows():
        # print(type(row))
        does_contain = True if form_name in row[1] else False
        # print(does_contain)
        if does_contain:
            relevant_rows.append(row.copy())
    
    res_df = pd.DataFrame(relevant_rows)

    return res_df


def _save_rules(df, form_name, file_name):
